...
static void init_tone (ToneMode_t mode,
		       uint32_t inc1,
		       uint32_t inc2,
		       uint32_t inc3,
		       uint32_t param1,
			  ...
			  uint32_t samples_per_channel)
{
  for (uint32_t i = 0; i < samples_per_channel; i++)
  {
    ...
    else if (inc_data.fade_stop)
    {
      ...
      apply_fade(&sample_left, &sample_right, fade_out);
...
//	interval_timer.step_start_tick = osKernelGetTickCount();
  ...
  switch (sinus)
  {
    case SINUS_420HZ_120S:
      init_tone(TONE_MODE_CONSTANT, PHASE_INC_420, PHASE_INC_420, 0, 120000, 0);
      break;
    case SINUS_1000HZ_120S:
      init_tone(TONE_MODE_CONSTANT, PHASE_INC_1000, PHASE_INC_1000, 0, 120000, 0);
      break;
    case SINUS_1020HZ_120S:
      init_tone(TONE_MODE_CONSTANT, PHASE_INC_1020, PHASE_INC_1020, 0, 120000, 0);
      break;
    case SINUS_20000HZ_120S:
      init_tone(TONE_MODE_CONSTANT, PHASE_INC_20000, PHASE_INC_20000, 0, 120000, 0);
      break;
    case SINUS_836HZ_856HZ_60S:
      init_tone(TONE_MODE_CONSTANT, PHASE_INC_836, PHASE_INC_836, 0, 60000, 0);
      break;
    case SINUS_ALARM_90S:
      init_tone(TONE_MODE_CYCLE, PHASE_INC_250, PHASE_INC_500, PHASE_INC_300, 3000, 84000);
      break;
    case SINUS_ALL_CLEAR_60S:
      init_tone(TONE_MODE_CYCLE, PHASE_INC_250, PHASE_INC_425, 0, 2000, 56000);
      break;
    case SINUS_ALL_CLEAR_120S:
      init_tone(TONE_MODE_CYCLE, PHASE_INC_250, PHASE_INC_425, 0, 2000, 116000);
      break;
    case SINUS_ABC_120S:
      init_tone(TONE_MODE_CYCLE, PHASE_INC_250, PHASE_INC_500, PHASE_INC_300, 500, 89000);
      break;
    case SINUS_400HZ_800HZ_1_5S_60S:
      init_tone(TONE_MODE_CYCLE_NORAMP, PHASE_INC_400, PHASE_INC_800, 0, 1500, 56000);
      break;
    case SINUS_400HZ_800HZ_3S_60S:
      init_tone(TONE_MODE_CYCLE_NORAMP, PHASE_INC_400, PHASE_INC_800, 0, 3000, 54000);
      break;
    default:
		       ...
		       uint32_t param2)
{
  switch (mode)
  {
    case TONE_MODE_CONSTANT:
      inc_data.samples_per_phase[0] = 0;
      inc_data.samples_per_phase[1] = 0;
      inc_data.samples_per_phase[2] = (uint64_t) SAMPLE_RATE * param1 / 1000;

      inc_data.duration = inc_data.samples_per_phase[2];
      inc_data.current_step = 1;
      inc_data.stable_middle = true;
      inc_data.current_inc = inc1;
      inc_data.target_inc = inc1;

      inc_data.add_inc[0] = 0;
      inc_data.add_inc[1] = 0;

      set_increment(inc1, inc2);
      break;

    case TONE_MODE_CYCLE:
      inc_data.target_phase[0] = inc1;
      inc_data.target_phase[1] = inc2;
      inc_data.target_phase[2] = inc3;

      inc_data.samples_per_phase[0] = (uint64_t) SAMPLE_RATE * param1 / 1000;
      inc_data.samples_per_phase[1] = (uint64_t) SAMPLE_RATE * (param1 / 2) / 1000;
      inc_data.samples_per_phase[2] = (uint64_t) SAMPLE_RATE * param2 / 1000;

      inc_data.duration = (inc_data.samples_per_phase[0] * 2) + inc_data.samples_per_phase[2];
      inc_data.current_step = 0;
      inc_data.stable_middle = (inc3 == 0);
      inc_data.current_inc = inc1;
      inc_data.target_inc = inc2;

      inc_data.add_inc[0] = (inc2 - inc1) / ((SAMPLE_RATE * param1) / 1000);
      inc_data.add_inc[1] = (inc2 - inc3) / ((SAMPLE_RATE * (param1 / 2)) / 1000);

      set_increment(inc1, inc1);
      break;

    case TONE_MODE_CYCLE_NORAMP:
      inc_data.target_phase[0] = inc1;
      inc_data.target_phase[1] = inc2;
      inc_data.target_phase[2] = inc1;
      ...
      inc_data.duration = inc_data.samples_per_phase[2];
      inc_data.current_step = 1;
      inc_data.stable_middle = false;
      inc_data.current_inc = inc1;
      inc_data.target_inc = inc2;

      inc_data.add_inc[0] = (inc2 - inc1) / ((SAMPLE_RATE * param1) / 1000);
      inc_data.add_inc[1] = inc_data.add_inc[0];
				    ...
				    uint32_t current_sample_in_buffer)
{
  ...
  switch (inc_data.current_step)
  {
    case STEP_RAMP_UP:
      ...
      inc_data.target_inc =
	  inc_data.stable_middle ? inc_data.target_phase[1] : inc_data.target_phase[2];
      break;

    case STEP_CYCLE:
      if (!inc_data.stable_middle)
      {
	inc_data.target_inc = inc_data.target_phase[
	    (inc_data.target_inc == inc_data.target_phase[2]) ? 1 : 2];
      }
    ...
    case STEP_RAMP_DOWN:
    {
      ...
      if (bytes_to_clear > 0)
      {
	memset(buffer + (current_sample_in_buffer * 4) + 4, 0, bytes_to_clear);
      }
      player->duration = 100;
      player->is_prepare_stoped = true;
      return true;
